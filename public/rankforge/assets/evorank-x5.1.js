...
(() => {
  ...
  function chart(points,label,unit,{integerAxis=false}={}) {
    ...
    const rawMaximum = Math.max(1,...points.map(p=>p.value))*1.08;
    // X6.1: runde Achsenwerte (z. B. 0 · 6 000 · 12 000) statt krummer Zahlen.
    const nice = v => { const p = 10**Math.floor(Math.log10(v)), n = v/p; return (n<=1?1:n<=2?2:n<=2.5?2.5:n<=5?5:10)*p; };
    const step = integerAxis?Math.max(1,Math.ceil(rawMaximum/3)):nice(rawMaximum/3);
    const count = Math.max(1,Math.ceil(rawMaximum/step)), maximum = step*count;
    const ticks=Array.from({length:count+1},(_,i)=>i/count);
    const gid = 'x61g' + (++chartCount);
    ...
    return `<svg class="x51-chart" viewBox="0 0 560 226" role="img" aria-label="${esc(label)}; Zeitachse von ${date(first)} bis ${date(last)}; ${esc(unit)}"><title>${esc(label)} · einzelne Aufzeichnungen nach Datum</title>${ticks.map(n=>`<path class="x51-grid" d="M${left} ${bottom-n*(bottom-top)}H${right}"/><text x="54" y="${bottom-n*(bottom-top)+4}" text-anchor="end">${num(n*maximum)}</text>`).join('')}<text x="${left}" y="12">${esc(unit)}</text>${points.length>1?`<defs><linearGradient id="${gid}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="var(--accent)" stop-opacity=".32"/><stop offset="1" stop-color="var(--accent)" stop-opacity="0"/></linearGradient></defs><path class="x61-area" fill="url(#${gid})" d="M${x(points[0]).toFixed(2)} ${bottom} ${points.map(p=>`L${x(p).toFixed(2)} ${y(p).toFixed(2)}`).join(' ')} L${x(points.at(-1)).toFixed(2)} ${bottom}Z"/><path class="x51-line" d="${points.map((p,i)=>`${i?'L':'M'}${x(p).toFixed(2)} ${y(p).toFixed(2)}`).join(' ')}"/>`:''}${points.map((p,i)=>`<circle class="x51-point${i===points.length-1?' x61-last':''}" cx="${x(p)}" cy="${y(p)}" r="${i===points.length-1?5.5:3.5}"><title>${date(p.time)} · ${num(p.value)} ${esc(unit)} · ${esc(p.name)}</title></circle>`).join('')}<text x="${left}" y="211">${date(first)}</text>${first!==last?`<text x="${right}" y="211" text-anchor="end">${date(last)}</text>`:''}</svg>`;
  }
  ...
  function trend(points) {
    ...
    const change = Math.round((points.at(-1).value/points[0].value-1)*100);
    const since = new Date(points[0].time).toLocaleDateString('de-AT',{day:'2-digit',month:'2-digit'});
    return `<b class="x61-trend ${change>0?'is-up':change<0?'is-down':''}">${change>0?'+':''}${change} % seit ${since}</b>`;
  }
  ...
  proto.handleSubmit=async function(event,...args){
    ...
    if(form?.dataset.form==='profile'&&form.elements.x51Weight){
      ...
      const data=new FormData(form), height=String(data.get('x51Height')||'').trim(), weightText=String(data.get('x51Weight')||'').trim(), weight=weightText?Number(weightText):null, birth=String(data.get('x51Birth')||'');
      ...
      if((weight!=null&&(weight<30||weight>250||!Number.isFinite(weight)))||(height&&(!Number.isFinite(Number(height))||Number(height)<80||Number(height)>250))||(birth&&age==null)||(age!=null&&(!Number.isFinite(age)||age<13||age>120))){this.showToast('Bitte gültige Körperdaten eintragen (Alter 13–120).');return;}
      Object.assign(this.state.profile,{heightCm:height?Number(height):null,bodyweightKg:weight??this.state.profile.bodyweightKg??null,birthDate:birth,age:age==null?null:Math.floor(age),ageReferenceDate:birth||age==null?'':window.EVORANK_AGE_X47.dayKey(new Date()),ageReferenceValue:birth||age==null?null:Math.floor(age)});
