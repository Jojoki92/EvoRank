-- =============================================================================
-- EVORANK 10.10 – Einwilligung, Moderation und geschützte Bestenlisten
-- Reihenfolge: zuerst die vorhandenen 9.8-, 10.8- und 10.9-SQL-Dateien,
-- danach diese Datei vollständig im Supabase SQL Editor ausführen.
-- =============================================================================
...
begin
  ...
  if v_source = 'garmin' and v_sport <> 'strength' and not exists (
    select 1 from public.rf_garmin_activities
    where user_id = auth.uid() and sport = v_sport
  ) then raise exception 'garmin_activity_not_verified'; end if;

  select score into v_previous_score
  from public.evorank_leaderboard_entries
  where user_id = auth.uid() and sport = v_sport;
  if v_source = 'manual' and v_previous_score is not null and p_score > v_previous_score + 2500 then
    v_moderation := 'review';
  end if;
